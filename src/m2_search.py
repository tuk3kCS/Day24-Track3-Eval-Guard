    ...
    def _get_encoder(self):
        if self._encoder is None:
            ...
            try:
                ...
                print(f"  [WARN] Failed to load dense embedding model '{EMBEDDING_MODEL}': {e}")
                fallback = "all-MiniLM-L6-v2"
                print(f"  [FALLBACK] Falling back to cached model '{fallback}'...")
                self._encoder = SentenceTransformer(fallback)
